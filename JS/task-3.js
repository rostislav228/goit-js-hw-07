...
// для создания тегов img вложенных в li.Для создания
...
const addsGalleryMarkup = (element, array) => {
  {
    const galleryMarkup = array
      .map(({ url, alt }) => `<img class="image" src="${url}" alt="${alt}">`)
      .join('');
    element.insertAdjacentHTML('beforeend', galleryMarkup);
  }
};

addsGalleryMarkup(galleryEl, images);
